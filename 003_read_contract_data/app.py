...
def connect():
    ...
    if is_connected:
        print(f'Connection made to Ethereum blockchain. Latest block is {latest_block}.')
    else:
        print('Connection Failed')


def get_contract(address, abi):
    ''' Get representation of smart contract
    ...
    contract_total_supply_wei = contract.functions.totalSupply().call()
    contract_total_supply = web3.fromWei(contract_total_supply_wei, 'ether')
    print(f'{contract_name}({contract_symbol}) has total supply {contract_total_supply}')
